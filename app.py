...
def add_customer():
    ...
    phone = request.form["phone_numbet"]
    addr = request.form["customer_address"]
    db.session.add(Customer(first_name=first, last_name=last, phone_numbet=phone, customer_address=addr))
    db.session.commit()
...
def invoices():
    ...
    return render_template("invoices.html", invoices=invoices, customers=Customer.query.all(), products=Product.query.all())
